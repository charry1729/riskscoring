...
class news_class():
    def __init__(self, current_time, website, stop, k=None):
        ...
        # if website in ["newsbtc","thebitcoinnews"]:
        if website in ["cryptoslate"]:
...
class thebitcoinnews(news_class):
    def scraper(self):
        ...
        try:
            ...
            print(soup)
            for post in soup.find_all('div', class_ = 'entry-title td-module-title'):
...
class newsbtc(news_class):
    ...
    def scraper(self):
        links = []
        urls = ['https://www.newsbtc.com/news/bitcoin',
                'https://www.newsbtc.com/bitcoin-news/',
                'https://www.newsbtc.com/news/',
                'https://www.newsbtc.com/tech/']
        ...
        try:
            ...
            if soup is not None:
                posts = soup.find('div', class_='jeg_post_excerpt')
                # print(posts)

                with open('output.html', 'w', encoding='utf-8') as file:
                    file.write(str(posts))
                link = soup.find('div', class_='jeg_meta_date').find('a').get('href')
                # print(link)
                with open('output1.html', 'w', encoding='utf-8') as file:
                    file.write(str(link))


                # Check if 'posts' is not None
                # if posts is not None:
                #     for post in posts: # .find_all('div', class_='jeg_post_excerpt'):
                #         link = soup.find('div', class_='jeg_post_meta').get('href')
                #         if link == self.news_latest:
                #             break
                #         else:
                #             links.append(link)
                ...
                #         return self.process_news(links)
                # else:
                #     print("No 'div' with class 'jeg_post_excerpt' found in the HTML structure.")
        except Exception as e:
    ...
    #             'https://www.newsbtc.com/news/',
    #             'https://www.newsbtc.com/tech/']
    #     #    'https://www.newsbtc.com/categories/crypto-tech/',
    #     #    'https://www.newsbtc.com/categories/industry-news/']
    #     # 'https://www.newsbtc.com/categories/crypto/'
    #     try:
    #         r = requests.get(urls[self.k])
    #         soup = BeautifulSoup(r.text, 'html.parser')
    #         # print("soup")
    ...
    #         print("posts")
    #         print(posts)
    #         for post in posts.findAll('div', class_ = 'post-content'):
...
class starter(object):
    ...
    def __init__(self, isNew):
        ...
        self.scrapers = [
                        #  scaper_bitcoinist,scaper_btcwires,scaper_ccn,scaper_coindesk,
                        #  scaper_coinspeaker,scaper_coinstaker,scaper_cointelegraph,scaper_cryptovest,
                         scaper_cryptoslate,
                         scaper_thebitcoinnews,
                         scaper_newsbtc_1,scaper_newsbtc_2,
